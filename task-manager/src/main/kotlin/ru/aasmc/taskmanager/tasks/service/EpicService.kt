...
class EpicService(
...
) : BaseService<EpicDto, Epic, EpicRepository>(
    epicRepository,
    validationService,
    eventService,
    epicMapper
    ...
    override fun validateTask(task: EpicDto) {
        mapper.toEntity(task).subtasks
            ?.map(validationService::requestValidation)
            ?.forEach { response ->
    ...
    fun getAllSubtasksOfEpic(id: Long): List<SubtaskDto> {
        ...
        return subsList.map(subtaskMapper::toDto)
    }
}